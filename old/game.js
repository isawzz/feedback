...
const Settings = {
	...
	INTERVAL: 1000 / FR_INIT,
	W_INIT: 50,
	PLUS: 10, //increment on click
	MINUS: 5,
	DECAY: 2, //decrement per second
	V_INIT: 1,
	V_MIN: 0.25,
	V_DECAY: .05, // decrement per sec
...
function calc_event(state, color) {
	record_event(state, color);
	state[color].width += color == 'green' ? Settings.PLUS : Settings.MINUS;
}

function record_event(state, color) {
	if (!E[color]) E[color] = {};
	let e = E[color];
	e.tlast = get_now();

	state[color].vel = Settings.V_INIT; //dec geht wieder schneller
}
