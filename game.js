...
function update_gamestate(state) {
	if (!state) { return; }
	for (const k of ['green', 'red']) if (state[k].pos > 0) calc_decay(state[k], (Settings.DECAY / Settings.FR), (Settings.VDECAY / Settings.FR));
	return false;
}
...
function calc_decay(st, dps, vps) {
	...
	//vps ... decrement of velocity v per second (VDECAY / FR)
	st.pos -= dps * st.v;
